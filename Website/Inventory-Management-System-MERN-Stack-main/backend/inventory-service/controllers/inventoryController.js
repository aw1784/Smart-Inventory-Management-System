...
exports.adjustQuantity = async (req, res) => {
  try {
    ...
    inventoryAdjustmentsTotal.inc();

    if (Math.abs(delta) > 100) {
      largeInventoryAdjustmentsTotal.inc();
    }
...
exports.adjustByProduct = async (req, res) => {
  try {
    ...
    if (!productId || typeof delta !== 'number') {
      return res.status(400).json({
        message: 'productId and numeric delta are required',
      });
    }
